...
export const TEXTAREA_SELECTOR = "#consoleSink";
 ...
 */
export function classifyReadiness(fullText: string): ReadinessState {
 ...
 */
export async function getConsoleLogStrings(driver: WebDriver): Promise<string[]> {
  try {
...
export function uploaderReadinessTest(driver: WebDriver): () => Promise<void> {
  return async () => {
